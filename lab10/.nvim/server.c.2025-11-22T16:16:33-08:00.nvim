...
Ans. The client is sending data to the server via the TCP/IP(SOCK_STREAM)
protocol. It connects to port 8001 on localhost.

2. What data is the client sending to the server?
Ans. The client is sending 5 messages to the server, which are: Hello, Apple,
Car, Green, Dog

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
Ans. The run_acceptor thread is passed as a struct acceptor_args containing run,
list_handle and list_lock

2. How are received messages stored?
Ans. The received messages are stored in a singly-linked list.

3. What does `main()` do with the received messages?
Ans. Maining waits for enough messages, stops the acceptor, validates count and
then calls collect_all() to print and free all messages.

4. How are threads used in this sample code?
Ans. There are three threads used in the sample code. The main thread
orchestrates execution and collects results. The acceptor thread accepts
connections and spawns client threads. The client threads handle one client's
incoming messages.

5. Explain the use of non - blocking sockets in this lab.
How are sockets made non - blocking?
Ans. Sockets are made non-blcoking using the fcntl() function with F_SETFL flag
and O_NONBLOCK option.

What sockets are made non - blocking?
Ans. The server and client sockets (sfd and cfd) are made non-blocking.

Why are these sockets made non - blocking?
Ans. The server socket is made non-blocking so that accept() is not blocked.
The acceptor thread can keep checking aargs->run flag in its loop without
getting stuck waiting for a client. Similarly, the client socket is made
non-blocking so that read() is not blocked.

What purpose does it serve?
Ans. The acceptor and client threads can check their run flags without getting
